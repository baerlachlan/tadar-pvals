...
res_A$mps %>%
  ...
  mutate(
    dar_q = cut(
      ...
      fct_na_value_to_level(levels(.)[[1]]),
    dar_q = str_split(dar_q, ",") %>%
      lapply(str_remove_all, pattern = "\\[|\\]|\\)|\\(") %>%
      # vapply(paste0, FUN.VALUE = character(1), collapse = " - ") %>%
      vapply(FUN.VALUE = character(1), \(x){
        as.numeric(x) %>%
          round(3) %>%
          paste0(collapse = " - ")
      }) %>%
  ...
  geom_histogram(fill = "grey70", colour = "black", binwidth = 0.05, linewidth = 0.5) +
  geom_label(
    aes(x = 1, y = 0, label = label),
    data = . %>%
      summarise(n = dplyr::n(), .by = dar_q) %>%
      mutate(label = paste("n =", comma(n))),
    hjust = 1, vjust = 0, alpha = 0.7, size = 3
  ) +
...
ggsave("~/phd/publications/tadar_manuscript/fig/p_dist_dar_q.png", width = 8, height = 6)
```
...
beta_fits_A$mps %>%
  ...
  geom_point(
    data = . %>% distinct(dar, shape1),
    size = 3, shape = 21, colour = "black", fill = "grey50"
  ) +
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  # geom_line(aes(y = estimate, colour = f))
  geom_smooth(method='lm', formula= y~x) +
  labs(x = "DAR", y = "\u03B1")
ggsave("~/phd/publications/tadar_manuscript/fig/alpha_dar.png", width = 8, height = 6)
```
...
res_A$mps %>%
  ...
  pivot_longer(
    cols = all_of(c("Before", "After")), names_to = "type", values_to = "p"
  ) %>%
  mutate(type = fct_relevel(type, "Before")) %>%
  ggplot(aes(sample = -log(p), colour = type)) +
  stat_qq(distribution = qexp, size = 3, alpha = 0.7) +
  stat_qq_line(distribution = qexp) +
  scale_y_continuous(breaks = seq(0, 50, 5)) +
  facet_wrap(~type) +
  # scale_colour_manual(values = c("Before" = pal[1], "After" = pal[2])) +
...
ggsave("~/phd/publications/tadar_manuscript/fig/qq_plot.png", width = 8, height = 6)
```
...
# ggsave("~/phd/publications/tadar_manuscript/fig/alpha_dar_post.png", width = 8, height = 6)
```
...
# ggsave("~/phd/publications/tadar_manuscript/fig/alpha_dar_prior.png", width = 8, height = 6)
```

```{r}
theme_set(theme_pubclean())
lm_all_nonorm %>%
  dplyr::select(dataset, intercept_Before = intercept, slope_Before = slope) %>%
  left_join(lm_all) %>%
  dplyr::select(dataset, intercept_Before, slope_Before, intercept_After = intercept, slope_After = slope) %>%
  pivot_longer(-dataset, names_to = c(".value", "Var"), names_sep = "_") %>%
  mutate(Var = fct_relevel(Var, "Before")) %>%
  ggplot() +
  geom_abline(aes(intercept = intercept, slope = slope)) +
  scale_x_continuous(limits = c(0, 1)) +
  scale_y_continuous(limits = c(0, 1.3)) +
